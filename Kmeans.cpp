...
double calcularProb(vector<double>& puntoEnX, vector<vector<double>>& vectorCentroides, double psi) {
	double prob = 0.0;
	double min = sqrt(calcDisMin(puntoEnX, vectorCentroides));
	prob = (min / psi);
	return prob;
}
...
void reclusterKmeans2(vector<vector<double>>& vectorCentroides, vector<vector<double>>& centroidesFinales, int hilos, int k) {
	...
	datosUsados.resize(tamCentroides);
#pragma omp parallel for num_threads(hilos) reduction(+:fCosto)
	for (int i = 0; i < tamCentroides; ++i) {
		fCosto += sqrt(calcDisMin(vectorCentroides[i], centroidesFinales));
	}
...
void reductionVectores(vector<vector<vector<double>>>& vCentroidesHilos, vector<vector<double>>& vectorCentroides, int hilos) {
	for (int i = 0; i < hilos; ++i) {
		if (i == 0)
			vectorCentroides.assign(vCentroidesHilos[i].begin(), vCentroidesHilos[i].end());
		else
			vectorCentroides.insert(vectorCentroides.end(), vCentroidesHilos[i].begin(), vCentroidesHilos[i].end());
	}
}
...
#pragma omp parallel for num_threads(hilos) reduction(+:fCosto)
	for (int i = 0; i < m; ++i) {
		fCosto += sqrt(calcDisMin(vectorDatos[i], vectorCentroides));
	}
	int contador = 0;
	...
	for (int i = 0; i < 5; ++i)
	{
		...
		# pragma omp parallel num_threads(hilos) shared(contador, vCentroidesHilos,datosUsados) private(escoger, posicionR)
		{
			while (contador < l) {
				escoger = generarDouble(0, 1);
				posicionR = generarInt(0, m - 1);
				if (escoger < probabilidades[posicionR]) {
					if (!datosUsados[posicionR]) {
							vCentroidesHilos[omp_get_thread_num()].push_back(vectorDatos[posicionR]);
							datosUsados[posicionR] = true;
							#pragma omp atomic
								++contador;
					}
				}
		...
		fCosto = 0;
		reductionVectores(vCentroidesHilos, vectorCentroides, hilos);
		#pragma omp parallel for num_threads(hilos) reduction(+:fCosto)
		for (int i = 0; i < m; ++i) {
			fCosto += sqrt(calcDisMin(vectorDatos[i], vectorCentroides));
		}
...
#pragma omp parallel for num_threads(hilos) shared(puntosAsociadosC) private(posMin) reduction(+:fCostoPrime)
		for (int i = 0; i < m; ++i) {
			fCostoPrime += sqrt(calcDisMin(vectorDatos[i], vectorCentroides));
			posMin = calcMinPos(vectorDatos[i], vectorCentroides);
#pragma omp critical
			{
				puntosAsociadosC[posMin].push_back(vectorDatos[i]);
